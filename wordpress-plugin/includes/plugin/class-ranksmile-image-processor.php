<?php
/**
 * Background image processor for Ranksmile.
 *
 * @package Ranksmile
 ...
 */
class Ranksmile_Image_Processor {

	/**
	 * Constructor.
	 */
	public function __construct() {
		add_action( 'ranksmile_process_image_queue', array( $this, 'process_image_queue' ) );
	}
	...
	public function process_image_queue() {
		...
		update_option( 'ranksmile_image_download_queue', array_values( $queue ) );

		if ( ! empty( $queue ) ) {
			wp_schedule_single_event( time() + 1, 'ranksmile_process_image_queue' );
		}
	}
	...
	private function download_and_replace_image( $image_url, $image_alt ) {
		...
		if ( false === $posts ) {
			...
			wp_cache_set( $cache_key, $posts, 'ranksmileseo_db', MINUTE_IN_SECONDS );
		}
	...
	private function download_image_to_media_library( $image_url, $image_alt ) {
		...
		update_post_meta( $attachment_id, 'ranksmile_file_name', $file_name_and_extension['file_name'] );
