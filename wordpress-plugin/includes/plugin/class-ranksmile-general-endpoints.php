...
/**
 * Object to reach Ranksmile endpoints with general usage.
 */
class Ranksmile_General_Endpoints {
	...
	/**
	 * Ajax points, that can be reachable from React front-end and make requests to Ranksmile.
	 */
	public function register_request_to_endpoints() {

		add_filter( 'wp_ajax_ranksmile_get_user_drafts', array( $this, 'get_user_drafts' ) );
		add_filter( 'wp_ajax_ranksmile_get_user_credits', array( $this, 'get_user_credits' ) );
		add_filter( 'wp_ajax_ranksmile_get_user_workspaces', array( $this, 'get_user_workspaces' ) );
	}

	/**
	 * Endpoints for Ranksmile general usage.
	 */
	public function register_general_api_endpoints() {
		...
		register_rest_route(
			...
			array(
				...
				'permission_callback' => function ( $request ) {
					return Ranksmile()->get_plugin()->verify_request_permission( $request );
				},
				'args'                => array(),
			)
		);
	...
	/**
	 * Returns user drafts from Ranksmile.
	 */
		...
		if ( 200 === $code || 201 === $code ) {
			...
			foreach ( $response['drafts'] as $draft ) {
				$drafts[] = array(
					...
					'editedInRanksmileDate'    => gmdate( 'd M Y H:i:s', strtotime( $draft['updated_at'] ) ),
	...
	/**
	 * Gets last edition date of post if it is synced with Ranksmile.
	 *
	 * @param array $draft - Draft from Ranksmile.
	 * @return string|null
	 */
	...
	/**
	 * Puts keywords from Ranksmile object to simple array.
	 *
	 * @param array $keywords - Keywords from Ranksmile.
	 * @return array
	 */
	...
	public function get_user_credits() {
		...
		list(
			'code'     => $code,
			'response' => $response,
		) = Ranksmile()->get_plugin()->make_ranksmile_request( '/get_organization_credits', array(), 'GET' );
	...
	/**
	 * Returns user workspaces from Ranksmile.
	 */
	public function get_user_workspaces() {
		...
		list(
			'code'     => $code,
			'response' => $response,
		) = Ranksmile()->get_plugin()->make_ranksmile_request( '/workspaces', array(), 'GET' );
	...
	public function get_available_post_types() {
		...
		$response = new WP_REST_Response( $post_types );
