 ...
 * Class to handle Drop Monitor related things.
 */
class Ranksmile_GSC_Drop_Monitor {

	use Ranksmile_GSC_Common;

	/**
	 * Number of rows to get from GSC.
	 * 1000 - for old Ranksmile endpoint.
	...
	public function init() {
		...
		add_action( 'ranksmile_gather_drop_monitor_data', array( $this, 'gather_position_monitor_data' ) );
		add_action( 'ranksmile_gather_position_monitor_data_bunch', array( $this, 'parse_data_bunch_for_position_monitor' ) );

		if ( ! wp_next_scheduled( 'ranksmile_gather_drop_monitor_data' ) ) {
			wp_schedule_single_event( strtotime( 'next monday 08:00:00' ), 'ranksmile_gather_drop_monitor_data' );
		}

		add_action( 'wp_ajax_ranksmile_test_gsc_traffic_gatherer', array( $this, 'test_gather_position_monitor_data' ) );

		add_action( 'wp_ajax_ranksmile_get_posts_for_performance_report', array( $this, 'get_posts_for_performance_report' ) );
		add_action( 'wp_ajax_ranksmile_get_domain_performance_report', array( $this, 'get_domain_performance_report' ) );

		add_action( 'wp_ajax_ranksmile_test_email_performance_report', array( $this, 'test_performance_email_template' ) );
	}
	...
	public function parse_data_bunch_for_position_monitor() {
		...
		$posts_max  = get_transient( 'ranksmile_gsc_data_collection_posts_max' );
		$posts_done = get_transient( 'ranksmile_gsc_data_collection_posts_done' );
		if ( false === $posts_done ) {
			$posts_done = 0;
		}
		...
		if ( $posts_done >= $posts_max ) {
			delete_transient( 'ranksmile_gsc_data_collection_posts_done' );
			delete_transient( 'ranksmile_gsc_data_collection_posts_max' );

			$this->send_performance_report_email();

			set_transient( 'ranksmile_gsc_weekly_report_ready', true, DAY_IN_SECONDS );
			update_option( 'ranksmile_last_gsc_data_update', gmdate( 'Y-m-d H:i:s', strtotime( 'this week monday ' . gmdate( 'H:i:s' ) ) ) );

			return 'All posts done for this week.';
		} else {
			// We may need another run.
			wp_schedule_single_event( strtotime( '+5 minute' ), 'ranksmile_gather_position_monitor_data_bunch' );
		}
		...
		if ( isset( $return['code'] ) && 200 === $return['code'] ) {
			foreach ( $return['response']['traffic_data'] as $i => $page ) {
				...
				++$posts_done;
				set_transient( 'ranksmile_gsc_data_collection_posts_done', $posts_done, HOUR_IN_SECONDS * 1 );
	...
	/**
	 * Execute query to get data from GSC via Ranksmile.
	 *
	...
	private function get_posts_from_gsc( $row_limit, $start_row ) {
		...
		return Ranksmile()->get_plugin()->make_ranksmile_request( '/get_traffic_from_gsc', $params );
	}
	...
	private function can_parse_data( $page ) {
		...
		if ( ! in_array( get_post_type( $post_id ), ranksmile_return_supported_post_types(), true ) ) {
			return 'Post with ID: ' . $post_id . ' has wrong post type: ' . get_post_type( $post_id );
		}

		$last_post_update    = $this->get_last_period_date( $post_id );
		$last_data_gathering = get_option( 'ranksmile_last_gsc_data_update', strtotime( 'this week monday' ) );
	...
	private function get_last_period_date( $post_id ) {
		...
		$records   = wp_cache_get( $cache_key, 'ranksmileseo_db' );

		if ( false === $records ) {
			$records = $wpdb->get_row(
				$wpdb->prepare(
					'SELECT p.data_gathering_date FROM ' . $wpdb->prefix . 'ranksmile_gsc_traffic AS p WHERE p.post_id = %d ORDER BY p.data_gathering_date DESC LIMIT 1',
					$post_id
				),
				ARRAY_A
			);
			wp_cache_set( $cache_key, $records, 'ranksmileseo_db', 5 * MINUTE_IN_SECONDS );
		}
	...
	private function parse_single_page_data( $page ) {
		...
		$post_id         = url_to_postid( $page['site'] );
		$previous_result = ranksmile_get_last_post_traffic_by_id( $post_id );
		...
		global $wpdb;
		$inset_result = $wpdb->insert( $wpdb->prefix . 'ranksmile_gsc_traffic', $data_to_insert );
	...
	public function gather_position_monitor_data() {
		...
		if ( ! Ranksmile()->get_plugin()->get_gsc()->check_if_gsc_connected( true ) ) {
		...
		if ( isset( $return['code'] ) && 200 === $return['code'] ) {
			...
			set_transient( 'ranksmile_gsc_data_collection_posts_max', $number_of_posts, HOUR_IN_SECONDS * 1 );
	...
	private function send_performance_report_email() {
		...
		$last_email_sent = get_transient( 'ranksmile_gsc_weekly_report_email_sent' );
		...
		$title   = sprintf( __( 'Ranksmile Performance Report for domain: %s', 'ranksmileseo' ), preg_replace( '/^https?:\/\//', '', home_url() ) );
		$headers = array(
		...
		if ( $email_sent ) {
			Ranksmile()->get_ranksmile_tracking()->track_wp_event( 'report_email_sent', home_url() );
			set_transient( 'ranksmile_gsc_weekly_report_email_sent', gmdate( 'm-d-Y H:i:s' ), 7 * DAY_IN_SECONDS );
		}
	}
	...
	public function test_performance_email_template() {
		...
		if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'ranksmile_send_test_performance_email' ) ) {
			return false;
		}
	...
	private function prepare_weekly_report_email_message() {
		...
		$last_update = get_option( 'ranksmile_last_gsc_data_update', false );
		...
		ob_start();
			require_once Ranksmile()->get_basedir() . '/templates/emails/performance-report.php';
	...
	private function get_posts_drops_in_top_10() {
		...
		$last_update_date = get_option( 'ranksmile_last_gsc_data_update', false );
		...
		$posts     = wp_cache_get( $cache_key, 'ranksmileseo_db' );

		if ( false === $posts ) {
			$posts = $wpdb->get_results(
				$wpdb->prepare(
					'SELECT * FROM ' . $wpdb->prefix . 'ranksmile_gsc_traffic AS t
			...
			AND t.data_gathering_date = ( SELECT MAX(data_gathering_date) FROM ' . $wpdb->prefix . 'ranksmile_gsc_traffic AS t2 WHERE t.post_id = t2.post_id )
			AND t.data_gathering_date >= %s
			ORDER BY t.position_change DESC',
					$query_last_date
				)
			);
			wp_cache_set( $cache_key, $posts, 'ranksmileseo_db', 5 * MINUTE_IN_SECONDS );
		}
	...
	/**
	 * Gets posts with position change at least +2 in top 50, if they were optimized in Ranksmile.
	 *
	 * @return array
	 */
		...
		if ( false === $posts ) {
			$posts = $wpdb->get_results(
				$wpdb->prepare(
					'SELECT * FROM ' . $wpdb->prefix . 'ranksmile_gsc_traffic AS t
			LEFT JOIN ' . $wpdb->prefix . 'posts AS p ON t.post_id = p.ID
			LEFT JOIN ' . $wpdb->prefix . 'postmeta AS meta ON p.ID = meta.post_id AND meta.meta_key = "ranksmile_draft_id"
			WHERE 
			  ( t.position + t.position_change ) < 50
			  AND t.position > 0
			  AND t.position_change IS NOT NULL
	...
	public function get_posts_for_performance_report() {
		...
		$last_update_date = get_option( 'ranksmile_last_gsc_data_update', 'this week monday' );
	...
	private function count_posts_for_performance_report( $query, $current_page, $per_page, $filter, $sorting, $count_only = false ) {
		...
		$cache_key = 'ranksmile_' . $current_page . '_' . $per_page . '_' . $filter . '_' . $sorting . '_' . $query . '_' . $count_only;
		$posts     = wp_cache_get( $cache_key, 'ranksmile' );
		...
		if ( false === $posts ) {
			...
			if ( $count_only ) {
				...
				$sql = 'SELECT p.ID, p.post_title, p.post_content, ranksmile_gsc.*, meta.meta_value AS draft_id';
			}

			$sql .= ' FROM ' . $wpdb->prefix . 'posts AS p';
			$sql .= ' JOIN ' . $wpdb->prefix . 'ranksmile_gsc_traffic AS ranksmile_gsc ON p.ID = ranksmile_gsc.post_id';

			if ( ! $count_only ) {
				$sql .= ' LEFT JOIN ' . $wpdb->prefix . 'postmeta AS meta ON meta.post_id = p.ID AND meta.meta_key = "ranksmile_draft_id"';
			}

			$sql .= ' WHERE post_type IN ("' . implode( '", "', ranksmile_return_supported_post_types() ) . '") AND post_status IN ("publish")';
			$sql .= ' AND data_gathering_date = ( SELECT MAX(data_gathering_date) FROM ' . $wpdb->prefix . 'ranksmile_gsc_traffic t2 WHERE ranksmile_gsc.post_id = t2.post_id )';
			...
			foreach ( $results  as $result ) {
				...
				$posts[] = array(
					...
					'permalinkHash'       => get_post_meta( $result->ID, 'ranksmile_permalink_hash', true ),
					'scrapeStatus'        => get_post_meta( $result->ID, 'ranksmile_scrape_ready', true ),
			...
			wp_cache_set( $cache_key, $posts, 'ranksmile', 60 * 5 );
		}
	...
	private function count_domain_performance_report_for_single_stat( $stat, $period = false ) {
		...
		$cache_key = 'ranksmile_domain_report_' . $stat . '_' . $period;
		$result    = wp_cache_get( $cache_key, 'ranksmile' );

		if ( false === $result ) {
			if ( 'prev' === $period ) {
				$sql = 'SELECT SUM(' . $stat . ' - ' . $stat . '_change) AS sum FROM ' . $wpdb->prefix . 'ranksmile_gsc_traffic t1';
			} else {
				$sql = 'SELECT SUM(' . $stat . ') AS sum FROM ' . $wpdb->prefix . 'ranksmile_gsc_traffic AS t1';
			}

			$sql .= ' WHERE data_gathering_date = ( SELECT MAX(data_gathering_date) FROM ' . $wpdb->prefix . 'ranksmile_gsc_traffic AS t2 WHERE t1.post_id = t2.post_id )';
			$sql .= $wpdb->prepare( ' AND data_gathering_date >= %s', $query_last_date );
			...
			wp_cache_set( $cache_key, $result, 'ranksmile', 60 * 5 );
		}
	...
	private function count_domain_performance_report_position_change( $direction, $period = false ) {
		...
		$cache_key = 'ranksmile_domain_report_' . $direction . '_' . $period;
		$result    = wp_cache_get( $cache_key, 'ranksmile' );
		...
		if ( false === $result ) {
			...
			if ( 'prev' === $period && 'fall' === $direction ) {
				$result = (int) $wpdb->get_var(
					$wpdb->prepare(
						'SELECT COUNT(*) FROM ' . $wpdb->prefix . 'ranksmile_gsc_traffic t1 WHERE data_gathering_date >= %s AND data_gathering_date < %s AND position_change > 0',
						$query_previous_date,
						$query_last_date
					)
				);
				...
				$result = (int) $wpdb->get_var(
					$wpdb->prepare(
						'SELECT COUNT(*) FROM ' . $wpdb->prefix . 'ranksmile_gsc_traffic t1 WHERE data_gathering_date >= %s AND position_change > 0',
						$query_last_date
					)
				);
			} else {
