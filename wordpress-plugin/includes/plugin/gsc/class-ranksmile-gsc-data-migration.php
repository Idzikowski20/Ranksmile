 ...
 * Class to handle data migration
 */
class Ranksmile_GSC_Data_Migration {
	...
	public function init() {
		add_action( 'wp_ajax_ranksmile_transfer_gsc_data_to_new_format', array( $this, 'transfer_gsc_data_to_new_format_force' ) );
	}
	...
	public function transfer_gsc_data_to_new_format() {
		...
		$archives  = wp_cache_get( $cache_key, 'ranksmileseo_db' );

		if ( false === $archives ) {
			$archives = $wpdb->get_results(
				$wpdb->prepare(
					'SELECT * FROM ' . $wpdb->prefix . 'postmeta WHERE meta_key = %s',
					'ranksmile_post_traffic_archive'
				)
			);
			wp_cache_set( $cache_key, $archives, 'ranksmileseo_db', MINUTE_IN_SECONDS );
		}
		...
		foreach ( $archives as $archove_row ) {
			...
			foreach ( $archive as $single_record ) {
				...
				$test         = $wpdb->insert( $wpdb->prefix . 'ranksmile_gsc_traffic', $data_to_insert );
