 ...
 */
class Ranksmile_Updater {

	/**
	 * Plugin basename, e.g. "ranksmileseo/ranksmileseo.php".
	 *
	 * @var string
	 */
	private $basename;
	...
	/**
	 * Plugin folder slug, e.g. "ranksmileseo".
	 *
	 * @var string
	 */
	private $slug;
	...
	public function __construct() {
		$this->basename = plugin_basename( RANKSMILE_PLUGIN_FILE );
	...
	private function get_remote_info() {
		$cached = get_transient( 'ranksmile_update_info' );
		if ( false !== $cached ) {
			return $cached;
		}

		$base = rtrim( Ranksmile()->get_plugin()->get_ranksmile_url(), '/' );
		...
		set_transient( 'ranksmile_update_info', $info, 6 * HOUR_IN_SECONDS );
		return $info;
	}
	...
	public function plugin_info( $result, $action, $args ) {
		...
		return (object) array(
			'name'          => isset( $info->name ) ? $info->name : 'Ranksmile',
			'slug'          => $this->slug,
	...
	public function clear_cache() {
		delete_transient( 'ranksmile_update_info' );
	}
}
