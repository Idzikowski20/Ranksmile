...
class Content_Importer {
	...
	public function __construct() {
		...
		$this->image_processor = new Ranksmile_Image_Processor();
	...
	public function register_ajax_actions() {
		...
		add_action( 'wp_ajax_ranksmile_pull_and_override_content', array( $this, 'pull_and_override_content' ) );
	}
	...
	public function save_data_into_database( $content, $args = array() ) {
		...
		try {
			...
			if ( ! is_wp_error( $post_id ) && isset( $args['draft_id'] ) ) {
				update_post_meta( $post_id, 'ranksmile_draft_id', $args['draft_id'] );
				update_post_meta( $post_id, 'ranksmile_permalink_hash', isset( $args['permalink_hash'] ) ? $args['permalink_hash'] : '' );
				update_post_meta( $post_id, 'ranksmile_keywords', $args['keywords'] );
				update_post_meta( $post_id, 'ranksmile_location', $args['location'] );
				update_post_meta( $post_id, 'ranksmile_scrape_ready', true );
				update_post_meta( $post_id, 'ranksmile_last_post_update', round( microtime( true ) * 1000 ) );
				update_post_meta( $post_id, 'ranksmile_last_post_update_direction', 'from Ranksmile to WordPress' );
			}
			...
			return new \WP_Error( 500, 'Ranksmile import failed: ' . $e->getMessage() );
		}
	}
	...
	private function resolve_post_author( $args, &$data ) {
		...
		if ( isset( $args['post_author'] ) && '' !== $args['post_author'] ) {
			...
			$default = Ranksmile()->get_ranksmile_settings()->get_option( 'content-importer', 'default_post_author', false );
	...
	private function resolve_post_status( $args, &$data ) {
		...
		if ( isset( $args['post_status'] ) && '' !== $args['post_status'] && in_array( $args['post_status'], $allowed_statuses, true ) ) {
			...
			$default = Ranksmileseo::get_instance()->get_ranksmile_settings()->get_option( 'content-importer', 'default_post_status', false );
	...
	private function resolve_post_category( $args, &$data ) {
		...
		if ( isset( $args['post_category'] ) && is_array( $args['post_category'] ) && ! empty( $args['post_category'] ) ) {
			...
			$default = Ranksmileseo::get_instance()->get_ranksmile_settings()->get_option( 'content-importer', 'default_category', false );
	...
	private function resolve_post_tags( $args, &$data ) {
		...
		if ( isset( $args['post_tags'] ) && is_array( $args['post_tags'] ) && count( $args['post_tags'] ) > 0 ) {
			...
			$default = Ranksmileseo::get_instance()->get_ranksmile_settings()->get_option( 'content-importer', 'default_tags', false );
	...
	private function resolve_post_meta_details( $args, &$data ) {
		...
		// Save in Ranksmile Meta to display.
		if ( 'ranksmile' === $chosen_seo_plugin ) {

			if ( isset( $args['meta_title'] ) && '' !== $args['meta_title'] ) {
				$data['meta_input']['_ranksmileseo_title'] = $args['meta_title'];
			}

			if ( isset( $args['meta_description'] ) && '' !== $args['meta_description'] ) {
				$data['meta_input']['_ranksmileseo_description'] = $args['meta_description'];
			}
		}
	}
	...
	private function find_active_seo_plugin() {
		...
		if ( ranksmile_check_if_plugins_is_active( 'wordpress-seo/wp-seo.php' ) ) {
			return 'yoast';
		} elseif ( ranksmile_check_if_plugins_is_active( 'all-in-one-seo-pack/all_in_one_seo_pack.php' ) ) {
			return 'aioseo';
		} elseif ( ranksmile_check_if_plugins_is_active( 'seo-by-rank-math/rank-math.php' ) ) {
			return 'rank_math';
		}
	...
	private function get_user_id_by_login( $login = false ) {
		...
		if ( false !== $user ) {
			$user_id = get_option( 'ranksmile_auth_user', 0 );
		}
	...
	public function pull_and_override_content() {
		...
		list(
			'code'     => $code,
			'response' => $response,
		) = Ranksmile()->get_plugin()->make_ranksmile_request( '/update_last_sync_date', $params );
