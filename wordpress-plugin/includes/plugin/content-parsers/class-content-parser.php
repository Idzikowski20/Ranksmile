...
namespace Ranksmile\Plugin\Content_Parsers;
...
class Content_Parser {
	 ...
	 *
	 * @param string $content - Content from Ranksmile.
	 * @return string
	 */
	...
	private function queue_image_download( $image_url, $image_alt ) {
		$queue   = get_option( 'ranksmile_image_download_queue', array() );
		$queue[] = array(
			'url'       => $image_url,
			'alt'       => $image_alt,
			'timestamp' => time(),
		);
		update_option( 'ranksmile_image_download_queue', $queue );

		if ( ! wp_next_scheduled( 'ranksmile_process_image_queue' ) ) {
			wp_schedule_single_event( time() + 5, 'ranksmile_process_image_queue' );
		}
	}
	...
	private function find_existing_attachment( $image_url ) {
		...
		$attachment_id = wp_cache_get( $cache_key, 'ranksmileseo_db' );

		if ( false === $attachment_id ) {
			$attachment_id = $wpdb->get_var(
				$wpdb->prepare(
					"SELECT post_id FROM {$wpdb->postmeta} 
			 WHERE meta_key = 'ranksmile_file_original_url' 
			 AND meta_value LIKE %s",
					$image_url
				)
			);
			wp_cache_set( $cache_key, $attachment_id, 'ranksmileseo_db', 5 * MINUTE_IN_SECONDS );
		}
	...
	/**
	 * Upload image from Ranksmile to WordPress and replace src to local one.
	 *
	...
	private function download_image_sync( $image_url ) {
		...
		update_post_meta( $attachment_id, 'ranksmile_file_name', $file_name );
		update_post_meta( $attachment_id, 'ranksmile_file_original_url', $image_url );
	...
	protected function handle_links_target_attribute( $doc ) {
		...
		$internal_links_rel    = Ranksmile()->get_ranksmile_settings()->get_option( 'content-importer', 'internal_links_rel', false );
		$internal_links_target = Ranksmile()->get_ranksmile_settings()->get_option( 'content-importer', 'internal_links_target', '_self' );
		$external_links_rel    = Ranksmile()->get_ranksmile_settings()->get_option( 'content-importer', 'external_links_rel', false );
		$external_links_target = Ranksmile()->get_ranksmile_settings()->get_option( 'content-importer', 'external_links_target', '_blank' );
		...
		foreach ( $links as $link ) {
			...
			if ( false !== strpos( $link_url, rtrim( get_home_url(), '/' ) ) ) {
				...
				if ( $internal_links_rel ) {
					...
					$link->setAttribute( 'rel', is_array( $internal_links_rel ) ? join( ' ', $internal_links_rel ) : $internal_links_rel );
				}
			}
			...
			if ( false === strpos( $link_url, rtrim( get_home_url(), '/' ) ) ) {
				...
				if ( $external_links_rel ) {
					$link->removeAttribute( 'rel' );
					$link->setAttribute( 'rel', is_array( $external_links_rel ) ? join( ' ', $external_links_rel ) : $external_links_rel );
				}
			}
		}
