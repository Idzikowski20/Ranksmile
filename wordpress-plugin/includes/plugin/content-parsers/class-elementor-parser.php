...
class Elementor_Parser extends Content_Parser {
	...
	/**
	 * Parse content from Ranksmile to Gutenberg Editor.
	 *
	...
	public function run_after_post_insert_actions( $post_id ) {
		...
		$page_template      = Ranksmile()->get_ranksmile_settings()->get_option( 'content-importer', 'default_page_template', 'default' );
		$elementor_template = Ranksmile()->get_ranksmile_settings()->get_option( 'content-importer', 'default_elementor_template', 'default' );
	...
	private function check_if_execute_recurrence( $node_type ) {
		...
		if ( in_array( $node_type, array( 'ul', 'ol', 'p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'h7', 'img', 'blockquote', 'table' ), true ) ) {
