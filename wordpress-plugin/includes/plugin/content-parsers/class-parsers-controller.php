...
if ( ! defined( 'RANKSMILE_ASYNC_THRESHOLD' ) ) {
	define( 'RANKSMILE_ASYNC_THRESHOLD', 8 );
}
...
class Parsers_Controller {
	...
	/**
	 * Parse content from Ranksmile into one of used editors.
	 *
	 * @param string $content - Content from Ranksmile.
	 * @return string
	 */
	public function parse_content( $content ) {
		$content        = apply_filters( 'ranksmile_pre_import_content_parsing', $content );
		$parsed_content = '';

		if ( apply_filters( 'ranksmile_skip_default_content_parsing', false ) ) {
		...
		return apply_filters( 'ranksmile_import_content_parsing', $parsed_content, $content );
	}
	...
	private function choose_parser() {
		...
		$config_parser = Ranksmile()->get_ranksmile_settings()->get_option( 'content-importer', 'default_content_editor', self::GUTENBERG );
	...
	private function if_user_is_using_classic_editor() {
		...
		$if_gutenberg_is_active         = ranksmile_check_if_plugins_is_active( 'gutenberg/gutenberg.php' );
		$if_disable_gutenberg_is_active = ranksmile_check_if_plugins_is_active( 'disable-gutenberg/disable-gutenberg.php' );
		$if_classic_editor_is_active    = ranksmile_check_if_plugins_is_active( 'classic-editor/classic-editor.php' );
	...
	private function if_user_is_using_elementor() {
		...
		$if_elementor_is_active = ranksmile_check_if_plugins_is_active( 'elementor/elementor.php' );
	...
	public function set_image_processing_mode( $image_count ) {
		$image_processing_mode = Ranksmile()->get_ranksmile_settings()->get_option( 'content-importer', 'image_processing_mode', 'auto' );

		if ( 'async' === $image_processing_mode || ( 'auto' === $image_processing_mode && $image_count > RANKSMILE_ASYNC_THRESHOLD ) ) {
