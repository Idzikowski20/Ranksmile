...
use DOMDocument;
use DOMElement;
use Ranksmile\Plugin;
use Ranksmile\Ranksmileseo;
use Ranksmile\Plugin\Ranksmile_Logger;
...
class Content_Exporter {
	...
	public function init() {
		...
		add_filter( 'wp_ajax_ranksmile_create_content_editor', array( $this, 'create_content_editor' ) );
		add_filter( 'wp_ajax_ranksmile_update_content_editor', array( $this, 'update_content_editor' ) );
		add_filter( 'wp_ajax_ranksmile_remove_post_draft_connection', array( $this, 'remove_post_draft_connection' ) );
		add_filter( 'wp_ajax_ranksmile_check_draft_status', array( $this, 'check_draft_status' ) );
		add_filter( 'wp_ajax_ranksmile_get_locations', array( $this, 'get_locations' ) );
		add_filter( 'wp_ajax_ranksmile_get_post_sync_status', array( $this, 'get_post_sync_status' ) );

		add_filter( 'wp_ajax_ranksmile_gather_posts_to_reconnect', array( $this, 'gather_posts_to_reconnect' ) );
		add_filter( 'wp_ajax_ranksmile_reconnect_posts_with_drafts', array( $this, 'reconnect_posts_with_drafts' ) );
		add_action( 'wp_ajax_ranksmile_remove_old_backups', array( $this, 'ranksmile_remove_old_backups' ) );
	}
	...
	public function add_export_content_button_to_posts_list( $actions, $post ) {
		$draft_id = get_post_meta( $post->ID, 'ranksmile_draft_id', true );

		if ( $draft_id ) {
			$actions['export_to_ranksmile'] = '<a href="' . Ranksmile()->get_plugin()->get_ranksmile_url() . '/drafts/' . intval( $draft_id ) . '" >' . __( 'Check in Ranksmile', 'ranksmileseo' ) . '</a>';
		}
	...
	/**
	 * Makes export content to Ranksmile.
	 */
	public function create_content_editor() {
		$json = file_get_contents( 'php://input' );
		$data = json_decode( $json );

		if ( ! ranksmile_validate_custom_request( $data->_ranksmile_nonce, 'ranksmile-ajax-nonce', false ) ) {
		...
		$meta_title       = isset( $data->post_id ) ? sanitize_text_field( wp_unslash( Ranksmile()->get_plugin()->get_post_meta_title( $data->post_id ) ) ) : false;
		$meta_description = isset( $data->post_id ) ? sanitize_text_field( wp_unslash( Ranksmile()->get_plugin()->get_post_meta_description( $data->post_id ) ) ) : false;
		...
		list(
			'code'     => $code,
			'response' => $response,
		) = Ranksmile()->get_plugin()->make_ranksmile_request( '/import_content', $params );

		if ( 200 === $code || 201 === $code ) {
			$this->save_post_ranksmile_details( $post_id, $params['keywords'], $params['location'], $response['id'], $response['permalink_hash'] );
		}
	...
	/**
	 * Makes update content to Ranksmile.
	 */
	public function update_content_editor() {
		...
		try {
			...
			$content          = isset( $data->content ) ? wp_kses_post( $this->parse_content_for_ranksmile( $data->content ) ) : false;
			...
			list(
				'code'     => $code,
				'response' => $response,
			) = Ranksmile()->get_plugin()->make_ranksmile_request( '/import_content_update', $params );

			if ( 200 === $code || 201 === $code ) {
				$logger->log_export( $original_content, $content, true );
				update_post_meta( $post_id, 'ranksmile_last_post_update', round( microtime( true ) * 1000 ) );
				update_post_meta( $post_id, 'ranksmile_last_post_update_direction', 'from WordPress to Ranksmile' );

				$this->save_post_ranksmile_details( $post_id, $keywords, $location, $draft_id, $permalink_hash );
			} else {
	...
	/**
	 * Saves Ranksmile details about post.
	 *
	 * @param int    $post_id - ID of the post.
	 * @param string $keyword - keyword for the post.
	 * @param string $location - location for the post.
	 * @param int    $draft_id - ID of the draft in Ranksmile.
	 * @param string $permalink_hash - hash of the post permalink.
	 * @return void
	 */
	private function save_post_ranksmile_details( $post_id, $keyword, $location, $draft_id, $permalink_hash ) {

		update_post_meta( $post_id, 'ranksmile_last_post_update', round( microtime( true ) * 1000 ) );
		update_post_meta( $post_id, 'ranksmile_last_post_update_direction', 'from WordPress to Ranksmile' );
		update_post_meta( $post_id, 'ranksmile_keywords', $keyword );
		update_post_meta( $post_id, 'ranksmile_location', $location );
		update_post_meta( $post_id, 'ranksmile_draft_id', $draft_id );
		update_post_meta( $post_id, 'ranksmile_permalink_hash', $permalink_hash );
	}
	...
	public function check_draft_status() {
		...
		list(
			'code'     => $code,
			'response' => $response,
		) = Ranksmile()->get_plugin()->make_ranksmile_request( '/check_draft_status', $params );
		...
		if ( 200 === $code || 201 === $code ) {
			...
			update_post_meta( $post_id, 'ranksmile_scrape_ready', $status );
		}
	...
	/**
	 * Parse content to match Ranksmile formatting, and keep whole HTML.
	 *
	 * @param string $content - content to parse.
	 * @return string
	 */
	private function parse_content_for_ranksmile( $content ) {
	...
	private function parse_dom_node( $parent_node, &$content ) {
		...
		foreach ( $parent_node->childNodes as $node ) {
			...
			// We need to get IMGs from <p> tag, to allow Ranksmile to handle this.
	...
	public function get_locations() {
		...
		echo wp_json_encode( Ranksmile()->get_plugin()->ranksmile_hardcoded_location() );
		wp_die();
	}
	...
	public function remove_post_draft_connection() {
		...
		list(
			'code'     => $code,
			'response' => $response,
		) = Ranksmile()->get_plugin()->make_ranksmile_request( '/disconnect_draft', $params );
	...
	/**
	 * Gets post sync status from WordPress and Ranksmile.
	 */
	public function get_post_sync_status() {
		...
		list(
			'code'     => $code,
			'response' => $response,
		) = Ranksmile()->get_plugin()->make_ranksmile_request( '/get_draft_sync_status', $params );

		if ( 200 === $code || 201 === $code ) {
			$response['code']                    = $code;
			$response['wp_last_update_date']     = get_the_modified_date( 'M d, Y H:i:s', $post_id );
			$response['keywords']                = get_post_meta( $post_id, 'ranksmile_keywords', true );
			$response['location']                = get_post_meta( $post_id, 'ranksmile_location', true );
			$response['ranksmile_last_update_date'] = gmdate( 'M d, Y H:i:s', strtotime( $response['ranksmile_last_update_date'] ) );

			$wp_last_sync_date = get_post_meta( $post_id, 'ranksmile_last_post_update', true );
			...
			if ( strtotime( $response['last_sync_date'] ) <= $wp_last_sync_date ) {
				...
				$response['last_sync_direction'] = get_post_meta( $post_id, 'ranksmile_last_post_update_direction', true );
			}
		}
	...
	public function gather_posts_to_reconnect() {
		...
		if ( ! check_ajax_referer( 'ranksmile-ajax-nonce', '_ranksmile_nonce', false ) ) {
			echo wp_json_encode( array( 'message' => 'Security check failed.' ) );
			wp_die();
		}
	...
	public function reconnect_posts_with_drafts() {
		...
		foreach ( $posts as $post_id ) {
			...
			$keyword  = get_post_meta( $post_id, 'ranksmile_keywords', true );
			...
			if ( ! isset( $drafts[ $draft_id ] ) ) {
				$response .= 'Post ' . $post_id . ' has no draft ' . $draft_id . ' in Ranksmile. Checked drafts: ' . count( $drafts ) . ' Search by keyword: ' . $keyword . PHP_EOL;
				continue;
			}
			$draft_details = $drafts[ $draft_id ];

			$this->save_post_ranksmile_details( $post_id, $draft_details['keyword'], $draft_details['location'], $draft_id, $draft_details['permalinkHash'] );
	...
	/**
	 * Searches for drafts in Ranksmile.
	 *
	 * @param int $post_id - ID of the post.
	 */
	private function search_for_drafts( $post_id ) {
		...
		) = Ranksmile()->get_plugin()->make_ranksmile_request( '/get_user_drafts', $params );
	 ...
	 * Removes old backups.
	 */
	public function ranksmile_remove_old_backups() {
		...
		$posts = get_posts(
			array(
				'post_type'   => 'post',
				'post_status' => array( 'ranksmile-backup' ),
				'numberposts' => -1,
			)
		);
