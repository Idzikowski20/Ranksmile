...
async function load() {
  ...
  const state = await import('@/src/infrastructure/onboardingState');
  return { query, ...state };
}
...
describe('page tour state', () => {
  ...
  it.each([
    ...
    const { isDuplicateColumn } = await import('@/src/infrastructure/onboardingState');
  ...
  it('rethrows a non-duplicate ALTER failure instead of marking the schema ready', async () => {
    ...
    const { isPageTourSeen } = await import('@/src/infrastructure/onboardingState');
