jest.mock('@/src/infrastructure/identity/commentAccess', () => ({ assertCommentAccess: jest.fn(), getCommentAccessKind: jest.fn() }));
...
jest.mock('@/src/infrastructure/http/commentBus', () => ({ emitCommentChange: jest.fn(), onCommentChange: jest.fn(() => () => {}) }));
jest.mock('../../database/database', () => ({ __esModule: true, default: { query: jest.fn(), sync: jest.fn().mockResolvedValue(undefined) } }));

import db from '../../database/database';
import { assertCommentAccess, getCommentAccessKind } from '@/src/infrastructure/identity/commentAccess';
