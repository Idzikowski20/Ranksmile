...
export function CompactSelect<T extends SelectKey = string>(props: CompactSelectProps<T>) {
  ...
  const defaultLabel = useMemo(() => {
    ...
    return (
      <>
        ...
        <span className="sentry-compact-select-badge">+{selectedOptions.length - 1}</span>
      </>
    );
  ...
  return (
    <div ref={wrapRef} className="sentry-compact-select" data-is-open={open || undefined}>
      {trigger ? (
      ...
      {open && (
        ...
        >
          {(menuTitle || (clearable && selectedOptions.length > 0)) && (
            <div className="sentry-compact-select-menu-header">
              {menuTitle && <span className="sentry-compact-select-menu-title">{menuTitle}</span>}
              {clearable && selectedOptions.length > 0 && (
                <button type="button" className="sentry-compact-select-clear" onClick={clear}>
                  Clear
                </button>
              )}
            </div>
          ...
          {searchable && (
            <div className="sentry-compact-select-search">
              <input
                autoFocus
                value={query}
          ...
          {!hideOptions && (
            <div className="sentry-compact-select-list styled-scrollbar">
              {filtered.length === 0 ? (
                <div className="sentry-compact-select-empty">{emptyMessage}</div>
              ) : filtered.map((item, idx) => {
                if (isSection(item)) {
                  return (
                    <div key={item.key ?? idx} className="sentry-compact-select-section">
                      {item.label && <div className="sentry-compact-select-section-label">{item.label}</div>}
