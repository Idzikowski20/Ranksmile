...
const TabBtn = styled.button<{ $on: boolean }>(({ $on }) => ({
  ...
  padding: '4px 12px',
  border: 'none', cursor: 'var(--koala-cursor-pointing)',
  borderRadius: 5,
  fontFamily: 'var(--font-family-primary)',
  fontSize: 14, fontWeight: 500,
  background: 'transparent',
  color: $on ? 'var(--koala-text-primary)' : 'var(--koala-text-secondary)',
  transition: 'color 160ms cubic-bezier(0.72, 0, 0.16, 1)',
  whiteSpace: 'nowrap',
  outline: 'none',
  '&:focus-visible': { outline: '2px solid var(--koala-focus)', outlineOffset: -2 },
}));

const Cnt = styled.span<{ $on: boolean }>(({ $on }) => ({
  fontWeight: 400, fontSize: 13,
  color: $on ? 'var(--koala-text-secondary)' : 'var(--koala-text-tertiary)',
}));
