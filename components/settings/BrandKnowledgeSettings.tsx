...
const BrandKnowledgeSettings = () => {
  ...
  return (
    <form onSubmit={save} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 24, width: '100%' }}>
      ...
      {/* Writing examples and the GOLD / BAD corpus were removed from this page on
          purpose: learning the brand's voice is meant to happen on the backend, not
          be a form the user fills in. The endpoints (/api/wie/brand-dna, /api/wie/corpus)
          and every lib behind them are untouched.
          ponytail: ceiling = nothing calls them automatically yet, so DNA stops being
          learned until a pipeline step does it; upgrade = trigger the learn pass from
          deep-analysis, where the article corpus already exists. */}
      <div>
