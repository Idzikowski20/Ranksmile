      ...
      {data.map((d, i) => {
        ...
        if (isHorizontal) {
          ...
          if (stacked && stackOffsets) {
            ...
            const leftX = scale(offset) ?? 0;
            const rightX = scale(offset + value) ?? 0;
            const gapOffset = seriesIndex * stackGap;
            x = leftX + gapOffset;
            barW = Math.max(0, rightX - leftX);
          ...
          if (stacked && stackOffsets) {
            const offset = stackOffsets.get(i)?.get(dataKey) ?? 0;
            const topY = scale(offset + value) ?? 0;
            const bottomY = scale(offset) ?? innerHeight;
            // stackGap shifts subsequent segments up and trims non-top segments
            const gapOffset = seriesIndex * stackGap;
            y = topY - gapOffset;
            barHeight = Math.max(0, bottomY - topY);
            if (!isLastSeries && stackGap > 0) {
