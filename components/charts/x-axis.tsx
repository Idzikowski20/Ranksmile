...
export interface XAxisProps {
  ...
  /** Hide the leftmost tick label to avoid colliding with Y-axis values. */
  ...
  const labelsToShow = useMemo(() => {
    ...
    if (tickMode === "domain") {
      ...
      if (projectionExtendsScale && xDomain != null) {
        ticks = appendProjectionTailTicks(
          dataTicks,
          data,
          xAccessor,
          xScale,
    ...
    if (!hideFirstLabel || ticks.length <= 1) {
      return ticks;
    }
    const minX = Math.min(...ticks.map((t) => t.x));
    return ticks.filter((t) => t.x > minX + 0.5);
  }, [
    tickMode,
    xDomain,
    data,
