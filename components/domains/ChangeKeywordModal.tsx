...
const KwValueCell = ({ children }: { children: React.ReactNode }) => (
   <div style={{ width: 92, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '12px 16px', fontSize: 14, color: 'var(--koala-text-secondary)', fontFamily: KW_FONT }}>
      {children}
   </div>
);
   ...
   return (
      <div onClick={onSelect} className="kw-row" style={{ display: 'flex', alignItems: 'stretch', borderBottom: '1px solid var(--koala-bg-secondary)', cursor: 'pointer', background: selected ? 'var(--koala-bg-secondary)' : 'var(--koala-bg-primary)', transition: 'background 100ms ease' }}>
         <label style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', cursor: 'pointer' }}>
            ...
            {suggest && (
               <span style={{ flexShrink: 0, padding: '2px 8px', borderRadius: 8, background: 'color-mix(in srgb, var(--koala-text-brand) 12%, transparent)', color: 'var(--koala-text-brand)', fontSize: 12, fontWeight: 600, fontFamily: KW_FONT }}>suggest</span>
            )}
         </label>
         {!addNew && (
...
function ChangeKeywordModal({ article, allKeywords, onClose, onSave }: {
   ...
   return (
      <ShellPortal>
      <div style={{ position: 'fixed', inset: 0, zIndex: overlayZ.modal, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.4)' }} onClick={onClose}>
         <div
            onClick={(e) => e.stopPropagation()}
            style={{ background: 'var(--koala-bg-primary)', borderRadius: 16, width: 840, maxWidth: 'calc(100vw - 32px)', maxHeight: 'calc(100vh - 48px)', display: 'flex', flexDirection: 'column', boxShadow: '0px 24px 64px rgba(0,0,0,0.2)', animation: 'growOut 0.2s cubic-bezier(0.16,1,0.3,1)', transformOrigin: 'center', fontFamily: KW_FONT }}
         >
            {/* Header */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24, padding: 24 }}>
               <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
                  <h2 style={{ margin: 0, fontSize: 20, fontWeight: 600, color: 'var(--koala-text-primary)', fontFamily: KW_FONT }}>Change main keyword</h2>
                  <button
                     type="button"
                     onClick={onClose}
                     aria-label="Close"
                     style={{ display: 'inline-flex', border: 'none', background: 'transparent', color: 'var(--koala-text-secondary)', cursor: 'pointer', padding: 0, transition: 'opacity 150ms ease' }}
               ...
               <p style={{ margin: 0, fontSize: 14, color: 'var(--koala-text-secondary)', fontFamily: KW_FONT, lineHeight: '20px' }}>
                  Select a keyword you are targeting with this page:{' '}
                  <strong style={{ color: 'var(--koala-text-primary)' }}>{article.title}{article.url ? ` (${article.url})` : ''}</strong>
               </p>
            </div>
            ...
            <div style={{ flexShrink: 1, overflowY: 'auto', padding: '0 24px', maxHeight: 312 }}>
               <div style={{ overflow: 'hidden' }}>
                  ...
                  <div style={{ position: 'sticky', top: 0, zIndex: 1, display: 'flex', alignItems: 'stretch', background: 'var(--koala-bg-primary)', borderBottom: '1px solid var(--koala-bg-secondary)' }}>
                     <div style={{ flex: 1, minWidth: 0, padding: '12px 16px', fontSize: 14, color: 'var(--koala-text-secondary)', fontFamily: KW_FONT }}>Keyword</div>
                     {['Position', 'Clicks', 'Impr.'].map((h) => (
                        <div key={h} style={{ width: 92, flexShrink: 0, padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', fontSize: 14, color: 'var(--koala-text-secondary)', fontFamily: KW_FONT }}>{h}</div>
                     ))}
                  </div>
                  ...
                  {!search.trim() && (suggestions.length > 0 || suggestLoading) && (
                     <>
                        <div style={{ padding: '12px 16px 6px', fontSize: 11, fontWeight: 600, color: 'var(--koala-text-secondary)', fontFamily: KW_FONT, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                           Suggested{suggestLoading && <span style={{ fontWeight: 400, color: 'var(--koala-text-tertiary)', textTransform: 'none', letterSpacing: 0 }}> loading…</span>}
                        </div>
                        {suggestions.map((s) => {
                  ...
                  {filtered.length === 0 && !searchIsNew ? (
                     <div style={{ padding: '24px 16px', textAlign: 'center', fontSize: 13, color: 'var(--koala-text-tertiary)', fontFamily: KW_FONT }}>
            ...
            <div style={{ display: 'flex', width: '100%', alignItems: 'center', gap: 12, padding: '16px 24px 0' }}>
               <svg viewBox="0 0 20 20" width="20" height="20" style={{ flexShrink: 0, color: 'var(--koala-text-primary)' }}>
               ...
               <div style={{ fontSize: 14, color: 'var(--koala-text-primary)', fontFamily: KW_FONT, lineHeight: '20px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 16, padding: 24 }}>
               <button
                  type="button"
                  onClick={onClose}
                  style={{ border: 'none', background: 'transparent', color: 'var(--koala-text-secondary)', fontSize: 16, fontWeight: 600, fontFamily: KW_FONT, cursor: 'pointer', padding: 0, transition: 'color 150ms ease' }}
               ...
               <button
                  ...
                  onClick={handleSave}
                  disabled={!canSave}
                  style={{ padding: '8px 24px', borderRadius: 8, border: 'none', background: canSave ? 'var(--koala-bg-inverse)' : 'var(--koala-bg-secondary)', color: canSave ? 'var(--koala-text-on-brand)' : 'var(--koala-text-tertiary)', fontSize: 16, fontWeight: 600, fontFamily: KW_FONT, cursor: canSave ? 'pointer' : 'default', transition: 'background 150ms ease' }}
                  onMouseEnter={(e) => { if (canSave) e.currentTarget.style.background = 'var(--koala-btn-brand-bg)'; }}
                  onMouseLeave={(e) => { if (canSave) e.currentTarget.style.background = 'var(--koala-bg-inverse)'; }}
               >
                  {saving ? 'Saving…' : 'Save'}
               </button>
            </div>
