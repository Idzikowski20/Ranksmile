...
const Thinking = ({ text }: { text: string }) => {
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      ...
      {open && (
        <div style={{ marginTop: 6, paddingLeft: 10, borderLeft: '2px solid #f0f0f2', color: 'var(--koala-text-tertiary)', fontSize: 12.5, lineHeight: 1.55, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
          {text}
        </div>
      )}
    </div>
  ...
  if (role === 'user') {
    return (
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <div
          style={{
            maxWidth: '88%', background: 'var(--koala-bg-secondary)', color: 'var(--koala-text-primary)',
