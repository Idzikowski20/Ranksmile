          ...
          <AnimatePresence initial={false}>
            {comments.map((c) => (
              <motion.div key={c.id} layout initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, height: 0 }}
                ...
                {c.quote && (
                  <div style={{ borderLeft: '2px solid #783AFB', padding: '2px 0 2px 8px', margin: '0 0 6px', fontSize: 12, color: '#71717b', fontStyle: 'italic', lineHeight: '17px', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{c.quote}</div>
                )}
