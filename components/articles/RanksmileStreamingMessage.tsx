...
const CARET_CSS = `
@keyframes ranksmile-stream-caret {
  0%, 49% { opacity: 1; }
  50%, 100% { opacity: 0; }
}
.ranksmile-stream-caret {
...
@keyframes ranksmile-stream-glow {
  from { filter: blur(0.4px); opacity: 0.72; }
  to { filter: none; opacity: 1; }
}
.ranksmile-stream-body.is-live .ranksmile-md {
  animation: ranksmile-stream-glow 0.28s ease-out;
}
`;

/**
 * Live Ranksmile reply — grows with SSE deltas, ChatGPT-style caret + soft entrance.
 * Reveals text through a paced buffer so large model chunks don't appear as whole sections.
 */
export default function RanksmileStreamingMessage({ text, streaming = true }: Props) {
  ...
  return (
    ...
    >
      ...
      <div ref={glowRef} className="ranksmile-stream-body">
        <RanksmileMarkdown>{visible}</RanksmileMarkdown>
      </div>
      {showCaret ? <span className="ranksmile-stream-caret" aria-hidden /> : null}
    </motion.div>
  );
}
