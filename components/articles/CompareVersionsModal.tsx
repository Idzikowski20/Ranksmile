...
const tagStyle = (tag: string): React.CSSProperties => {
  switch (tag) {
    case 'h1': return { fontSize: 24, fontWeight: 800, color: 'var(--koala-text-primary)', margin: '14px 0 8px', lineHeight: 1.2 };
...
function highlightTerms(text: string, regex: RegExp | null): React.ReactNode {
  ...
  while (m !== null) {
    ...
    out.push(
      <span key={`t${n}`} style={{ textDecoration: 'underline', textDecorationColor: 'var(--koala-text-brand)', textDecorationThickness: 2, textUnderlineOffset: 2 }}>{m[0]}</span>,
    );
...
const Pane = ({ blocks, side, regex, innerRef, onScroll }: { blocks: DiffBlock[]; side: 'left' | 'right'; regex: RegExp | null; innerRef: React.RefObject<HTMLDivElement>; onScroll: () => void }) => (
  <div ref={innerRef} onScroll={onScroll} style={{ flex: 1, minWidth: 0, border: '1px solid var(--koala-border-primary)', borderRadius: 8, padding: '12px 16px', overflowY: 'auto' }} className="styled-scrollbar">
    {blocks.map((b, i) => {
...
const CompareVersionsModal = ({ original, updated, terms, onClose }: Props) => {
  ...
  return createPortal(
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: overlayZ.modal, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, fontFamily: F }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 1100, maxHeight: '88vh', background: 'var(--koala-bg-primary)', borderRadius: 12, display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 24px 70px rgba(0,0,0,0.35)' }}>
        {/* Header */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px 12px' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 600, color: 'var(--koala-text-primary)' }}>Compare versions</h2>
          <button type="button" onClick={onClose} aria-label="Close" style={{ border: 'none', background: 'transparent', cursor: 'pointer', color: 'var(--koala-text-secondary)', display: 'inline-flex', padding: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 16, padding: '0 24px 10px' }}>
          <span style={{ flex: 1, fontSize: 12, fontWeight: 600, letterSpacing: '0.04em', color: 'var(--koala-text-tertiary)' }}>ORIGINAL</span>
          <span style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 12, fontWeight: 600, letterSpacing: '0.04em', color: 'var(--koala-text-tertiary)' }}>
            <span>NEW</span>
            <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, cursor: 'pointer', textTransform: 'none', letterSpacing: 0, fontWeight: 400, color: 'var(--koala-text-secondary)', fontSize: 13 }}>
              <span
                role="switch"
              ...
              >
                <span style={{ position: 'absolute', top: 2, left: highlight ? 16 : 2, width: 16, height: 16, borderRadius: '50%', background: 'var(--koala-bg-primary)', transition: 'left 0.15s', boxShadow: '0 1px 2px rgba(0,0,0,0.25)' }} />
              </span>
              Highlight terms
            </label>
          </span>
        ...
        <div style={{ borderTop: '1px solid #f4f4f5', padding: 16, display: 'flex', justifyContent: 'flex-end' }}>
          <button type="button" onClick={onClose} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '10px 20px', borderRadius: 8, border: 'none', background: 'var(--koala-bg-inverse)', color: 'var(--koala-text-on-inverse)', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: F, transition: 'background 0.15s' }}
