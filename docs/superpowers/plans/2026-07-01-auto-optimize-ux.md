...
- **Part 8 — presentation + re-SCORE only, no re-PLAN.** Do NOT touch `lib/optimizationPlanner.ts`, `lib/optimizeGuidelineRouting.ts`, `lib/recommendationEngine.ts`, `lib/aiSearchScore.ts`, `buildArticleContext`, or the endpoint planner/ROI/credit/abort/retry logic. **Do NOT modify `lib/aiCoverage.ts` or `lib/contentScore.ts` internals** — import and reuse `computeCoverageScores` / `computeContentScore` / `countOccurrences` only. The NEW pure `liveCoverage.ts` re-scorer REUSES them; it never re-plans. **[Open item #2 — RATIFIED] The `structure`/`readability`/`paa` per-ITEM presence checks are small LOCAL pure functions in `liveCoverage.ts`. The `contentScore.ts` `_readability`/`_listUsage`/`_faqCoverage` helpers are document-level PARTIAL scorers (return 0–N, whole-doc) — wrong granularity for a per-item boolean, so they are NOT reused (and `contentScore.ts` stays untouched). Only `countOccurrences` (per-item, already exported) is reused, for `entity`, matching `contentScore.ts:336` exactly.**
...
- [ ] Create `lib/liveCoverage.ts` importing types + the two reused pure scorers ONLY (`import type { CoverageItem, CoverageType, BucketScore } from './aiCoverage'`; `import { computeCoverageScores } from './aiCoverage'`; `import { countOccurrences } from './contentScore'`). Do NOT import or alter `contentScore`/`aiCoverage` internals — the `structure`/`readability`/`paa` checks are LOCAL per-item pure functions (see `presenceCovered` below).
