...
import { compileArticle, resolveCompileSource } from '@/src/core/intelligence/runtimeApi';
import type { CompileStore } from '@/src/core/intelligence/compileStore';
import { recordCcmCompileMetric, type CcmCompileOutcome } from '@/src/core/intelligence/ccmCompileMetrics';
...
async function resolveStore(explicit?: CompileStore): Promise<CompileStore> {
  ...
  const { SqlCompileStore } = await import('@/src/core/intelligence/sqlCompileStore');
...
async function projectCoverageSafe(
  articleId: number,
  model: import('@/src/core/ccm/types/ccm').CanonicalContentModel,
  createdAt: string,
): Promise<CoverageSnapshot | undefined> {
  const { persistCcmCoverageProjection } = await import('@/src/core/intelligence/persistCcmCoverageProjection');
...
export async function compileAfterArticleChange(
  ...
  try {
    ...
    if (opts.enrichDaFacts !== false) {
      try {
        const { applyDaFactEnrichment } = await import('@/src/core/intelligence/applyDaFactEnrichment');
