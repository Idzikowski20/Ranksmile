...
async function resolveOrgId(req: NextApiRequest, res: NextApiResponse): Promise<ResolvedOrg> {
  ...
  if (userId) {
    try {
      const { ensureUserTenancy } = await import('@/src/infrastructure/tenancy');
  ...
  try {
    const { resolveByApiKey } = await import('@/src/infrastructure/wpConnection');
...
async function loadAccessSnapshot(orgId: number, userId: string | null): Promise<AccessSnapshot> {
  const { getOrgBillingState } = await import('@/src/infrastructure/orgBilling');
  ...
  if (userId) {
    ...
    try {
      const { getConfirmationStatus } = await import('@/src/infrastructure/emailConfirmation');
      emailConfirmed = (await getConfirmationStatus(userId)).confirmed;
    } catch {
      emailConfirmed = true;
    }
    try {
      const { getCallerRole } = await import('@/src/infrastructure/members');
...
export function withOrgPaymentAccess(handler: NextApiHandler): NextApiHandler {
  return async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (urlPath === '/api/billing/tax-preview' && method === 'POST') {
      ...
      try {
        const { assertCanManage } = await import('@/src/infrastructure/members');
