...
function resolveStore(explicit?: FeatureStore): FeatureStore {
  ...
  if (process.env.JEST_WORKER_ID || process.env.NODE_ENV === 'test') {
    const { createMemoryFeatureStore } = require('@/src/infrastructure/featureStoreCore') as typeof import('@/src/infrastructure/featureStoreCore');
  ...
  try {
    ...
    const { createDbFeatureStore } = require('@/src/infrastructure/featureStore') as typeof import('@/src/infrastructure/featureStore');
...
export async function persistCoverageFeatureRun(opts: {
  ...
  const recordEvent = async (
    ...
    try {
      const { persistDomainEvent } = await import('@/src/infrastructure/growthMetaStore');
