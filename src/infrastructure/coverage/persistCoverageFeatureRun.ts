...
export async function persistCoverageFeatureRun(opts: {
  ...
  const recordEvent = async (
    ...
    try {
      const { persistDomainEvent } = await import('@/src/infrastructure/stores/growthMetaStore');
  ...
  if (!(process.env.JEST_WORKER_ID || process.env.NODE_ENV === 'test')) {
    try {
      ...
      const { persistKnowledgeLayer } = await import('@/src/infrastructure/stores/growthMetaStore');
