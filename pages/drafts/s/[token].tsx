...
const SharePreviewPage: NextPage = () => {
  ...
  const author = useMemo(() => ({ name: authorName || 'Guest', color: '#F84416' }), [authorName]);
  ...
  const shareControl = (
    <div ref={shareRef} style={{ position: 'relative', display: 'inline-flex' }}>
      <button type="button" onClick={() => setShareOpen((v) => !v)}
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '6px 14px', borderRadius: 6, border: 'none', background: shareOpen ? '#F84416' : '#18181b', color: '#fff', fontSize: 13, fontWeight: 600, fontFamily: F, cursor: 'pointer', flexShrink: 0, transition: 'background 0.15s' }}
        onMouseEnter={(e) => { e.currentTarget.style.background = '#F84416'; }} onMouseLeave={(e) => { e.currentTarget.style.background = shareOpen ? '#F84416' : '#18181b'; }}>
        Share
      </button>
      {shareOpen && (
  ...
  return (
    <div style={{ height: '100vh', overflow: 'hidden', background: '#f4f4f5', fontFamily: F, display: 'flex', flexDirection: 'column' }}>
      ...
      {isEditing && (
        <div style={{ position: 'fixed', bottom: 12, left: 12, zIndex: 60, padding: '4px 10px', borderRadius: 999, background: 'rgba(242,153,100,0.12)', color: '#F84416', fontSize: 12 }}>✏️ editing…</div>
      )}
      ...
      <style>{`
        ...
        .preview-prose a { color: #F84416; text-decoration: underline; }
          ...
          <div className="styled-scrollbar" style={{ flex: 1, minWidth: 0, minHeight: 0, background: '#fff', borderRadius: 12, border: '1px solid #e4e4e7', overflow: 'auto' }}>
            <div ref={wrapperRef} style={{ position: 'relative', maxWidth: 820, margin: '0 auto', padding: '40px 64px 80px' }}>
              <div ref={contentRef} style={{ position: 'relative' }}>
                ...
                {caretBox && (
                  <span aria-hidden style={{ position: 'fixed', left: caretBox.left, top: caretBox.top, height: caretBox.height, width: 2, background: '#F84416', pointerEvents: 'none', animation: 'art-caret-blink 1s step-end infinite', zIndex: 50 }} />
                )}
              </div>
              <CommentsLayer
