...
const RankTrackingPage: NextPage = () => {
  ...
  const gscConnectHref = `/api/gsc/connect?redirect=${encodeURIComponent(`/sites/${slug}/rank-tracking`)}`;
  ...
  return (
    <AppShell domains={domains} showAddModal={() => {}} showSettings={() => {}}>
      <Head><title>{`Search Intelligence — ${domain} — SerpBear`}</title></Head>
      <style>{`
      ...
      <DomainSubLayout
        ...
        contentMaxWidth="100%"
        actions={(
          <Link href={trackerHref} style={{ textDecoration: 'none' }}>
