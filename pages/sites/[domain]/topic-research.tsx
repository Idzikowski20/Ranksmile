...
import {
   useTopicResearchList, useTopicResearchStatus, useCreateTopicResearch, useRunTopicResearch, useDeleteTopicResearch,
} from '../../../services/topicResearch';
...
const TopicResearchPage: NextPage = () => {
   ...
   const listQ = useTopicResearchList(slug);
   const statusQ = useTopicResearchStatus(slug);
   const createM = useCreateTopicResearch(slug);
   const runM = useRunTopicResearch(slug);
   const deleteM = useDeleteTopicResearch(slug);
   ...
   const openDetail = (id: number) => { router.push(`/sites/${slug}/topic-research/${id}`); };
   ...
   return (
      <AppShell domains={domains} showAddModal={() => {}} showSettings={() => {}}>
         <Head><title>{`Topic Research — ${domain}`}</title></Head>
         <style>{'@keyframes spin{to{transform:rotate(360deg)}}'}</style>
         <DomainSubLayout domain={domain} slug={slug || ''} section="Topic Research" contentMaxWidth="100%">
            <div style={{ width: '100%', maxWidth: 880, margin: '0 auto' }}>
               <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 20 }}>
                  <h1 style={{ margin: 0, fontSize: 18, fontWeight: 600, color: '#09090B', fontFamily: FONT }}>Topic Research</h1>
               ...
               <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  {!loading && items.length === 0 && (
                     <div style={{ border: '1px solid #F4F4F5', borderRadius: 12, background: '#fff', padding: '56px 24px', textAlign: 'center' }}>
                        ...
                        <div style={{ fontSize: 15, fontWeight: 600, color: '#18181B', fontFamily: FONT }}>No topic research yet</div>
                        <div style={{ fontSize: 13, color: '#71717B', fontFamily: FONT, marginTop: 4 }}>Enter a seed topic to discover clusters and content ideas.</div>
                     </div>
                  )}
