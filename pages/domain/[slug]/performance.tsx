...
function MetricCard({
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: 8, borderRadius: 8, background: accentBg, color: accentColor }}>
        <EyeIcon muted={muted} />
      </div>
    </div>
  );
...
function LineChart({ data }: { data: ChartPoint[] }) {
  ...
  const clickPath = data.map((item, index) => `${index === 0 ? 'M' : 'L'}${getX(index).toFixed(3)},${getClicksY(item.clicks).toFixed(3)}`).join('');
  const impressionPath = data.map((item, index) => `${index === 0 ? 'M' : 'L'}${getX(index).toFixed(3)},${getImpressionsY(item.impressions).toFixed(3)}`).join('');
  ...
  const hoveredClicksY = hoveredPoint ? getClicksY(hoveredPoint.clicks) : null;
  const hoveredImpressionsY = hoveredPoint ? getImpressionsY(hoveredPoint.impressions) : null;
  ...
  if (hoverState && hoveredClicksY !== null && hoveredImpressionsY !== null) {
    const rect = containerRef.current?.getBoundingClientRect();
    if (rect) {
      const plotTopPx = (topPad / svgHeight) * rect.height;
      const plotHeightPx = (innerHeight / svgHeight) * rect.height;
      const anchorYPx = plotTopPx + (Math.min(hoveredClicksY, hoveredImpressionsY) / innerHeight) * plotHeightPx;
      ...
      <svg width="100%" height="100%" viewBox={`0 0 ${svgWidth} ${svgHeight}`} preserveAspectRatio="none">
        <g transform={`translate(${leftPad},${topPad})`}>
          ...
          {[0, 1, 2, 3].map((index) => {
            const y = innerHeight - (innerHeight / 3) * index;
            return (
              <text key={`left-${index}`} x="-10" y={y} textAnchor="end" dominantBaseline="middle" fill="#52525C" fontSize="12" fontFamily="var(--font-family-primary)">
          ...
          {[0, 1, 2, 3].map((index) => {
            const y = innerHeight - (innerHeight / 3) * index;
            return (
              <text key={`right-${index}`} x={innerWidth + 10} y={y} textAnchor="start" dominantBaseline="middle" fill="#52525C" fontSize="12" fontFamily="var(--font-family-primary)">
          ...
          {hoverState && hoveredPointX !== null && hoveredClicksY !== null && hoveredImpressionsY !== null ? (
            <>
              <line
                x1={hoverState.lineX}
                x2={hoverState.lineX}
                ...
                strokeDasharray="6 6"
              />
              <circle cx={hoveredPointX} cy={hoveredClicksY} r="5.5" fill="#74A9FF" style={{ transition: 'cx 90ms linear, cy 90ms linear' }} />
              <circle cx={hoveredPointX} cy={hoveredImpressionsY} r="5.5" fill="#8B73F6" style={{ transition: 'cx 90ms linear, cy 90ms linear' }} />
            </>
          ) : null}
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 4 }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <span style={{ width: 8, height: 8, borderRadius: 9999, background: '#74A9FF' }} />
                <span>Clicks</span>
              </span>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 4 }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <span style={{ width: 8, height: 8, borderRadius: 9999, background: '#8B73F6' }} />
...
const PerformancePage: NextPage = () => {
  ...
  const [selectedDateRange, setSelectedDateRange] = useState<DateRangeValue>(() => getPresetRange('90'));
  const [calendarMonth, setCalendarMonth] = useState<Date>(() => startOfMonth(parseDateKey(getPresetRange('90').start)));
  ...
  return (
    <AppShell domains={domains} showAddModal={() => {}} showSettings={() => {}}>
      ...
      <DomainSubLayout domain={domain} slug={slug || ''} section="Performance" actions={feedbackAction} contentMaxWidth="unset">
        {isLoading ? (
          <div style={{ padding: '48px 0', textAlign: 'center', fontSize: 14, color: '#52525C', fontFamily: 'var(--font-family-primary)' }}>
          ...
          <div style={{ position: 'relative', display: 'flex', flex: 1, overflow: 'auto', borderRadius: 12, background: '#FFFFFF' }}>
            <div style={{ display: 'flex', width: '100%', flexDirection: 'column', gap: 32, paddingBottom: 16 }}>
              ...
              >
                                            ...
                                            return (
                                              <td
                                                ...
                                                style={{
                                                  ...
                                                  height: 38,
                                                  textAlign: 'center',
                                                  background: activeRange ? '#F4F4F5' : 'transparent',
                                              ...
                                              >
                                                <button
                                                  ...
                                                  style={{
                                                    ...
                                                    background: isStart || isEnd ? '#18181B' : 'transparent',
                ...
                <div className="performance-goal-bar" style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 1fr 1.2fr', gap: 16, padding: 24, color: '#3F3F47' }}>
