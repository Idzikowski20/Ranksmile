// POST /api/articles/ranksmile-agent
// Multi-step Ranksmile agent: read SEO tools + write tools over a server-side cheerio
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    try {
      ...
      if (isRanksmileReplyShape(parsedReply) && parsedReply) {
      ...
      console.log(`[ranksmile-agent] writes=${ctx.writeCount} tokens=${finalUsage?.totalTokens ?? runningTokens}`);
    ...
    console.error('[ranksmile-agent] error:', error);
