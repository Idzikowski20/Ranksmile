...
import { enqueueTopicResearch } from '../../../../lib/topicResearchRunner';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
   ...
   if (!seed) return res.status(400).json({ error: 'A topic seed is required' });
   const country = typeof body.country === 'string' ? body.country.trim().toUpperCase() : 'US';
   if (!country) return res.status(400).json({ error: 'Country is required' });

   try {
      const id = await enqueueTopicResearch(domainId, seed, country);
