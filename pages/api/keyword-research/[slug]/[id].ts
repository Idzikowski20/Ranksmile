...
async function handler(req: NextApiRequest, res: NextApiResponse) {
   ...
   if (!Number.isFinite(id)) return res.status(404).json({ error: 'Keyword research not found' });

   if (req.method === 'DELETE') {
      await db.query('DELETE FROM keyword_research_runs WHERE id = ? AND domain_id = ?', { replacements: [id, domainId] });
