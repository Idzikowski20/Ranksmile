...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
   ...
   const counts = await queryRows<CountRow>(
      'SELECT status, COUNT(*) AS n FROM audit_runs WHERE domain_id = ? GROUP BY status',
      [domainId],
   );
   ...
   const inFlight = await queryRows<InFlightRow>(
      "SELECT id, status, progress_done, progress_total FROM audit_runs WHERE domain_id = ? AND status IN ('queued','running') ORDER BY id ASC",
      [domainId],
   );
