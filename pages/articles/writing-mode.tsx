...
const WritingModePage: NextPage = () => {
  ...
  const goNext = () => {
    if (mode === 'write') {
      // "Write yourself" finishes the wizard → clear saved state, open the editor.
  ...
  const preview = (lines: number) => (
    <div style={{ width: 96, height: 109, background: '#E4E4E7', borderRadius: 8, padding: 3, flexShrink: 0 }}>
      <div style={{ width: '100%', height: '100%', background: '#fff', borderRadius: 6, padding: '12px 10px', display: 'flex', flexDirection: 'column', gap: 5, overflow: 'hidden' }}>
        {Array.from({ length: lines }).map((_, i) => (
          <div key={i} style={{ height: 3, borderRadius: 2, background: i % 4 === 2 ? '#D4D4D8' : '#EDEDF0', width: `${[100, 70, 90, 60, 85, 75][i % 6]}%` }} />
        ))}
      </div>
    </div>
  );
        ...
        {card(mode === 'generate', () => setMode('generate'), (
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 16, fontWeight: 500, color: '#000' }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="#F97316"><path d="M12.93 1.64a1 1 0 0 0-1.86 0L9.05 6.87c-.3.78-.4 1.01-.52 1.19-.13.18-.29.34-.47.47-.18.13-.41.22-1.19.52L1.64 11.07a1 1 0 0 0 0 1.86l5.23 2.01c.78.3 1.01.4 1.19.52.18.13.34.29.47.47.13.18.22.41.52 1.19l2.01 5.23a1 1 0 0 0 1.86 0l2.01-5.23c.3-.78.4-1.01.52-1.19.13-.18.29-.34.47-.47.18-.13.41-.22 1.19-.52l5.23-2.01a1 1 0 0 0 0-1.86l-5.23-2.01c-.78-.3-1.01-.4-1.19-.52a1.5 1.5 0 0 1-.47-.47c-.13-.18-.22-.41-.52-1.19L12.93 1.64Z" /></svg>
                Generate content
              </span>
              <p style={{ margin: 0, fontSize: 14, color: '#52525C' }}>Let AI create complete content optimized for AI Search and SEO.</p>
              <div onClick={(e) => { e.stopPropagation(); setCustomizeOpen((v) => !v); }} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 13, cursor: 'pointer' }}>
                <span style={{ color: '#52525C' }}>With internal and external links ·</span>
                <span style={{ color: '#000', display: 'inline-flex', alignItems: 'center', gap: 2 }}>
                  Customize
                  <svg width="16" height="16" viewBox="0 0 20 20" fill="currentColor" style={{ transform: customizeOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.15s' }}><path fillRule="evenodd" d="M5.22 8.22a.75.75 0 0 1 1.06 0L10 11.94l3.72-3.72a.75.75 0 1 1 1.06 1.06l-4.25 4.25a.75.75 0 0 1-1.06 0L5.22 9.28a.75.75 0 0 1 0-1.06" clipRule="evenodd" /></svg>
                </span>
              </div>
