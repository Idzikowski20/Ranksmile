...
const toIso = (d: any): string => { try { return new Date(d).toISOString(); } catch { return new Date().toISOString(); } };

/** Map our article row to the Surfer "draft" shape the WP plugin consumes (see get_user_drafts). */
export function articleToDraft(a: Row) {
   const kw = a.target_keyword || '';
   return {
      id: a.id,
      title: a.title || '',
      content: a.content || '',
      ...
      permalink_hash: permalinkHash(a.id),
      progress_snapshots: [{ name: 'content_score', value: a.content_score || 0 }],
...
export async function getArticleRow(id: number): Promise<Row | null> {
   const articleIdSql = await getArticleIdSql();
...
export async function listArticlesForDomain(domainId: number, query: string): Promise<Row[]> {
   const articleIdSql = await getArticleIdSql();
   let where = 'WHERE domain_id = ?';
   const repl: any[] = [domainId];
   if (query && query.trim()) {
